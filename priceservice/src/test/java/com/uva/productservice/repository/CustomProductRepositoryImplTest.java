...
class CustomProductRepositoryImplTest {
    ...
    @InjectMocks
    private CustomProductRepositoryImpl customProductRepository;
    ...
    void testFindAllByCriteria_AllParamsPresent() {
        ...
        when(criteriaBuilder.equal(root.get("description"), description)).thenReturn(descriptionPredicate);
        when(criteriaBuilder.equal(root.get("price"), price)).thenReturn(pricePredicate);
        when(criteriaBuilder.and(namePredicate, descriptionPredicate, pricePredicate)).thenReturn(combinedPredicate);
        when(criteriaQuery.where(combinedPredicate)).thenReturn(criteriaQuery);
        List<Product> expectedProducts = List.of(Product.builder().name(name).description(description).price(price).build());
    ...
    void testFindAllByCriteria_OnlyName() {
        ...
        when(typedQuery.getResultList()).thenReturn(List.of(Product.builder().name("Tacos al pastor").build()));
